...
- **HUD & game feel** — two-corner minimal HUD: top-left stack (HP pips, both weapons' ammo always shown — active bright, inactive dimmed, damage-upgrade marker once owned), bottom-left stats line `seed · room · kills · time` from `world.run` (#8). **Spread-aware reticle**: tight 4-tick crosshair for pulse, ±9° fan + ~91px-reach arc for scatter — makes #11's range split legible with no text. **Game-feel in**: small screen shake (2–4px decay, player hurt + tank charge impact only — graduates #3's deferred shake into #9's camera module), enemy-death additive particle pop, pickup collect-pop. **Cut**: ambient particles, projectile trails, hit-stop, edge flash, aim bias. **Room toast**: fading `ROOM n` banner on entry (~1.5s, doubles as spawn-room-exit confirmation), no persistent room UI. Details: `docs/hud-feel.md`.
